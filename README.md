...
| `silver.zoning_grid_columns`, `silver.lot_zoning_envelopes` | What a *grille des usages et des normes* states, at the zone's grain and joined to the pieces that zone governs. `piece_area_m2` is the ground a row is solved over and `lot_area_m2` the parcel it belongs to; `solver_ready` says whether the parse survived contact with the solver |
