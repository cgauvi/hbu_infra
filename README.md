...
| `silver.lot_assessed_values` | What each lot is assessed at, from Quebec's *rôle d'évaluation foncière* — the assessment units whose point falls inside the lot, summed on `rl0404a` (VALEUR IMMEUBLE). `num_assessment_units` travels beside the total because a divided co-ownership is one unit per apartment on one `PC-*` lot, so a total of $258M can be 402 units; `total_assessed_value` is **null**, not 0, on a lot no unit falls in — a lane is not worth nothing, it is unassessed |
