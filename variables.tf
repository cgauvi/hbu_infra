...
variable "app_db_username" {
  description = "Login role the pipeline and the query side connect as — created by the dataplatform's sql/pgvector_bootstrap.sql, which expects this exact name. It owns the `rag` schema and nothing else; the master user is only for bootstrap and migrations."
  type        = string
  default     = "urban_rag"
}
